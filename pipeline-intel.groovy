...
// Intel-specific configure flag transforms:
// - Append --with-intel to all configs
// - For GPU configs (those with gpus field), add explicit CUDA compiler paths
def intelTransform(Map configs) {
    def CUDA_FLAGS = '-D CMAKE_CUDA_COMPILER=/usr/local/cuda/bin/nvcc -D CMAKE_CUDA_HOST_COMPILER=/usr/bin/g++'
    def result = [:]
    configs.each { name, cfg ->
        def newCfg = new HashMap(cfg)
        def extra = ' --with-intel'
        if (cfg.gpus) {
            extra += " ${CUDA_FLAGS}"
        }
        newCfg.configure = cfg.configure + extra
        result[name] = newCfg
    }
    return result
}
        ...
        stage('Load Configs') {
            steps {
                script {
                    ...
                    def base = new HashMap(parsed)
                    base.each { k, v -> base[k] = new HashMap(v) }
                    charmmConfigs = intelTransform(base)
